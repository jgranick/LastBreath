...
@:font("04B_03.ttf") class GameFont extends flash.text.Font {}

@:sound("theme.mp3") class Music extends flash.media.Sound {}
@:sound("land.mp3") class Land extends flash.media.Sound {}
@:sound("grab.mp3") class Grab extends flash.media.Sound {}
@:sound("explode.mp3") class Explode extends flash.media.Sound {}
@:sound("die.mp3") class Die extends flash.media.Sound {}
@:sound("hit.mp3") class Hit extends flash.media.Sound {}
@:sound("jump1.mp3") class Jump1 extends flash.media.Sound {}
@:sound("jump2.mp3") class Jump2 extends flash.media.Sound {}
@:sound("jump3.mp3") class Jump3 extends flash.media.Sound {}
@:sound("land.mp3") class Respawn extends flash.media.Sound {}
@:sound("shadow.mp3") class Shadow extends flash.media.Sound {}
@:sound("pickup.mp3") class PickUp extends flash.media.Sound {}
@:sound("shard.mp3") class Shard extends flash.media.Sound {}
@:sound("fallLand.mp3") class FallLand extends flash.media.Sound {}
@:sound("trigger.mp3") class Trigger extends flash.media.Sound {}
@:sound("bump.mp3") class Bump extends flash.media.Sound {}
@:sound("think.mp3") class Think extends flash.media.Sound {}
//class Msg extends flash.media.Sound {}
...
class Manager {//}
	...
	static var SFX_LIB = new Map();
	static var MUSIC_VOLUME = 1;
	static var MAIN_VOLUME = 0.5;
	static var RNAMES = [
	...
	public function new(r) {
		...
		tiles.setGroup("lightRay");
		tiles.slice(0,192, 16*2, 16*7, 2);
		tiles.setGroup("lightColumn");
		tiles.slice(16*4,192, 16*5, 16*9);
		tiles.setGroup("lightSource");
		tiles.slice(16*9,192, 16*3, 16*2);
		tiles.setGroup("lightRadius");
		tiles.slice(16*9,192+16*2, 16*4, 16*4);
		
		tiles.setGroup("wall");
		tiles.setCenter(0,0);
		tiles.slice(0,0, 16,16, 4);
		tiles.setGroup("door");
		tiles.slice(16*4,0, 16,16);
		tiles.setGroup("ground");
		tiles.slice(0,16*1, 16,16, 2);
		tiles.setGroup("spike");
		tiles.slice(16*5,16*1, 16,16, 1);
		tiles.setGroup("wallBg");
		tiles.slice(16*6,16*1, 16,16, 4);
		tiles.setGroup("grass");
		tiles.slice(16*2,16*1, 16,16, 3);
		tiles.setGroup("props");
		tiles.slice(0,16*2, 16,16, 11);
		tiles.setGroup("ceilProps");
		tiles.slice(0,16*3, 16,16, 5);
		
		tiles.setGroup("trigger");
		tiles.slice(16*10,16*1, 16,16, 2);
		tiles.setGroup("key");
		tiles.slice(16*12,16*1, 16,16, 1);
		
		tiles.setGroup("iGrass");
		tiles.slice(0,16*26, 16*5,16);
		tiles.setGroup("iRoad");
		tiles.slice(16*5,16*26, 16*5,16);
		tiles.setGroup("iCar");
		tiles.slice(16*10,16*26, 16*2,16*2);
		tiles.setGroup("iBall");
		tiles.slice(16*12,16*26, 16,16, 2);
		tiles.setGroup("iSky");
		tiles.slice(0,16*27, 16*5,16*2, 2);
		
		tiles.setGroup("player");
		tiles.slice(0,16*5, 16,16, 20,5);
		tiles.setAnim("standing", [0,1,2], [3]);
		
		tiles.setAnim("walking", [20,21,22, 23,24,25, 26,27], [1]);
		tiles.setAnim("running", [20,22, 24, 26,27], [1]);
		//tiles.setAnim("jumpWalking", [40,41,42], [1]);
	...
	function onKey(e:flash.events.KeyboardEvent) {
		...
		if( k==Keyboard.SPACE || k==Keyboard.ENTER )
			closePop(true);
		if( phase=="cinematic" && (k==Keyboard.C || k==Keyboard.S) ) {
			curDialog = new Array();
			closePop();
			startGame();
		}
	...
	function updateGhostDist() {
		ghostDist = Std.int( Math.max(0.35, 1-shards.length/maxShards)*150 );
	}
	...
	function getCineFilters():Array<flash.filters.BitmapFilter> {
		return [
	...
	function playIntroCinematic() {
		...
		cine.addChild(grass);
		grass.filters = cast [ new flash.filters.DropShadowFilter(1,-90, 0x0,0.5, 1,1,10)];
		grass.y = 16*2-1;
		...
		cineUpdate = function() {
			...
			if( flags.get("inGarden") ) {
				...
				if( dog.getFrame()==63 && !flags.get("ballBounce1") ) {
					...
					onDialogEnd = function() {
						haxe.Timer.delay(playSound.bind(new Bump(), 0.1), 400);
						haxe.Timer.delay(playSound.bind(new Bump(), 0.3), 1300);
						haxe.Timer.delay(playSound.bind(new Bump(), 0.5), 2200);
						haxe.Timer.delay(playSound.bind(new Bump(), 0.3), 3100);
